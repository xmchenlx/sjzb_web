...
import { addNewArticle, getArticleInfo, editArticle } from '@/api/article'
...
export default {
  ...
  created: function () {
    this.loadArticleTag()
    this.getArticleById(utils.getUrlKey('aid'))
  },
  methods: {
    getArticleById (aid) {
      let _this = this
      getArticleInfo({'aId': aid}).then(res => {
        _this.title = res.data.data.aTitle
        _this.tagVal = '' + res.data.data.aTag
        // this.content = this.$refs.wTextArea.editorContent
        _this.$refs.wTextArea.editArticle(res.data.data.aContent)
      })
    },
    test () {
